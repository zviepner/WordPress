...
function addOrders() {
	...
	while ( $loop->have_posts() ) {
		...
		$order_xml->addAttribute("id", $order->get_id());
		...
		$order_xml->Status = $order->get_status();
		$order_xml->OriginalStatus = $order->get_status();
		$order_xml->CurrencyCode = $order->get_currency();
		$note_string = $order->get_customer_note();
		...
		$order_xml->Url = admin_url() . 'post.php?post=' . $order->get_id() . '&action=edit';
		...
		$billing_address->FirstName = htmlspecialchars($order->get_billing_first_name());
		$billing_address->LastName = htmlspecialchars($order->get_billing_last_name());
		$billing_address->OrganizationName = htmlspecialchars($order->get_billing_company());			
		$billing_address->Line1 = htmlspecialchars($order->get_billing_address_1());
		$billing_address->Line2 = htmlspecialchars($order->get_billing_address_2());
		$billing_address->City = htmlspecialchars($order->get_billing_city());
		$billing_address->PostCode = $order->get_billing_postcode();
		$billing_address->State = htmlspecialchars($order->get_billing_state());
		$billing_address->CountryCode = htmlspecialchars($order->get_billing_country());
		$shipping_address = $addresses_xml->addChild('Address');
		$shipping_address->addAttribute('type','Shipping');
		$shipping_address->FirstName = htmlspecialchars($order->get_shipping_first_name());
		$shipping_address->LastName = htmlspecialchars($order->get_shipping_last_name());
		$shipping_address->OrganizationName = htmlspecialchars($order->get_shipping_company());		
		$shipping_address->Line1 = htmlspecialchars($order->get_shipping_address_1());
		$shipping_address->Line2 = htmlspecialchars($order->get_shipping_address_2());
		$shipping_address->City = htmlspecialchars($order->get_shipping_city());
		$shipping_address->PostCode = $order->get_shipping_postcode();
		$shipping_address->State = htmlspecialchars($order->get_shipping_state());
		$shipping_address->CountryCode = htmlspecialchars($order->get_shipping_country());
		
		// Contact
		$contact_xml = $order_xml->addChild("Contact");
		if (($order->get_customer_id() != null) && ($order->get_customer_id() != "0")) {
			$contact_xml->addAttribute("id", $order->get_customer_id());
		}

		// Add contact details
		$contact_xml->FirstName = htmlspecialchars($order->get_billing_first_name());
		$contact_xml->LastName = htmlspecialchars($order->get_billing_last_name());
		$contact_xml->Email = $order->get_billing_email();
		$contact_xml->MobilePhone = $order->get_billing_phone();
		$contact_xml->OrganizationName = $order->get_billing_company();
		...
		foreach ($order->get_items() as $lineItem) {
			...
			if ($lineItem['variation_id'] != 0 && $lineItem['variation_id'] != NULL && $lineItem['variation_id'] != "") 
			{
				...
				$lineItem_xml->ProductName = $product->get_name() . '(';
		...
		if ($order->get_status() == "completed" || $order->get_status() == "processing" ) {
			...
			$payment_xml->addAttribute("Name",$order->get_payment_method_title());
			$payment_date = $order->get_date_paid();
			if(is_object($payment_date)) // use paid date if exists
			  {
			$payment_xml->Date = $payment_date->date('Y-m-d H-i-s');
			  }
			else // default to order date
			  {
			$payment_xml->Date = $loop->post->post_date_gmt;
			  }
